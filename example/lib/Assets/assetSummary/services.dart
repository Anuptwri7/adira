...
Future fetchAssetRfidSummary(int id) async {
  ...
  final response = await http.get(
      Uri.parse('https://$baseUrl${StringConst.assetRfidSummary+id.toString()}'),
      headers: {
...
Future fetchcodesumaary(int id) async {
  ...
  // log(sharedPreferences.getString("access_token"));
  final response = await http.get(
      Uri.parse('https://$baseUrl${StringConst.assetDetailMobile+id.toString()}'),
      headers: {
        'Content-type': 'application/json',
        'Accept': 'application/json',
        'Authorization': 'Bearer ${sharedPreferences.get("access_token")}'
