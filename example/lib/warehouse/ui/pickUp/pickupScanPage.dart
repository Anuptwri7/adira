...
import 'package:classic_simra/warehouse/ui/pickUp/model/summaryModel.dart';
import 'package:classic_simra/warehouse/ui/pickUp/sendPickUpDetail.dart';
...
class PickUpSummary extends StatefulWidget {
  int id;
  List SerialNo = [];
  PickUpSummary({Key key,this.id,this.SerialNo}) : super(key: key);

  @override
  State<PickUpSummary> createState() =>
      _PickUpSummaryState();
}
...
class _PickUpSummaryState extends State<PickUpSummary> {
  ...
  void initState() {
    fetchAssetRfidSummary(widget.id);
                      ...
                      child: Column(
                        children: [
                          FutureBuilder(
                            ...
                            future: fetchAssetSummary(widget.id.toString()),
                            ...
                            builder:
                              ...
                              if (snapshot.hasData) {
                                ...
                                try {
                                  log("fjshdkjfhsdkjf"+snapshot.data);
                                  final snapshotData = json.decode(snapshot.data);
                                  AssetSummary summary =
                                  AssetSummary.fromJson(snapshotData);
                                  // log(customerOrderList.count.toString());

                                  return summary.results[0].assetDispatchInfo.isEmpty?Text("Item ${summary.results[0].itemName} Has ALready been Returned!",style: TextStyle(color: Colors.red),):  DataTable(
                                      ...
                                      rows: List.generate(
                                          summary.results[0].assetDispatchInfoCount,
                                            ...
                                            cells: [
                                              DataCell(
                                                Padding(
                                                  ...
                                                  child: Text(
                                                    summary.results[0].assetDispatchInfo[index].rfidTag.code.toString(),
                                              ...
                                              DataCell(
                                                  GestureDetector(
                                                    onTap: (){
                                                      for (int i=0;i<summary.results[0].assetDispatchInfoCount;i++){
                                                        availablTagKey.add(summary.results[0].assetDispatchInfo[i].rfidTag.code);
                                                      }
                                                      log(availablTagKey.toString());
                                                      summary.results[0].picked==true?Fluttertoast.showToast(msg: "Already Picked"): Navigator.push(
                                                          context, MaterialPageRoute(builder: (context)=>SendPickupDetail(
                                                        id: summary.results[0].id,
                                                          rfidTag:availablTagKey),
                                                      ));
                                                    },
                                                    child: Text(
                                                      summary.results[0].picked==false?
                                                      "Click to scan Rfid Tag"
                                                          :'Picked'
                                                      ,
                                                      style:  TextStyle(
                                                        color: summary.results[0].picked==false?Colors.red:Colors.green ,
                                                          fontSize: 12,
                                                          fontWeight:
                                                          FontWeight.bold),
                                                    ),
