...
class SendPickupDetail extends StatefulWidget {
  int id;

  List rfidTag=[];

  SendPickupDetail({Key key,this.id,this.rfidTag}) : super(key: key);

  @override
  State<SendPickupDetail> createState() => _SendPickupDetailState();
}

class _SendPickupDetailState extends State<SendPickupDetail> {
...
List availableLocation = [];
  ...
  addDatas(List<RfidData> datas) async {
    for (var item in datas) {
      ...
      if(rfidDatas[item.tagID].peakRSSI>-40)
      {

        if(widget.rfidTag.contains(item.tagID)){
          if(rfidDocument.contains(item.tagID)){}else{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Send Pickup Details"),
      ),
...
for(int i =0;i<rfidDocument.length;i++){
  tagKey.add(
   rfidDocument[i].toString()
    ...
    final responseBody = {
      "asset_dispatch_detail": widget.id,
      "rfid_tag_codes":
       tagKey
      ,
    ...
    final response = await http.post(
        Uri.parse('https://$finalUrl${StringConst.sendPickupDetail}'),
        headers: {
    ...
    if (response.statusCode == 200||response.statusCode==201) {
      ...
      Fluttertoast.showToast(msg: "Picked Successfully");
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>PickupHomePage()));
      DataCell.empty;
